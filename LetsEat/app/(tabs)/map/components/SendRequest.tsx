...
const IncomingOpenJio: React.FC<IncomingOpenJioProps> = ({
  onRequestUpdate,
}) => {
  ...
  useEffect(() => {
    ...
    const requestsRef = ref(
      FIREBASE_DB,
      `users/${currentUser.uid}/openJioRequests`
    );
    onValue(requestsRef, (snapshot) => {
      ...
      if (data) {
        ...
        const uniqueRequests = openJioRequests.filter(
          (request, index, self) =>
            index ===
            self.findIndex((r) => r.requesterUid === request.requesterUid)
        );
  ...
  const handleOpenJioAccept = async (id: string) => {
    ...
    try {
      ...
      if (!eventExists) {
        ...
        const senderEventsRef = ref(
          FIREBASE_DB,
          `users/${requesterUid}/events`
        );
      ...
      const requestsRef = ref(
        FIREBASE_DB,
        `users/${currentUserUid}/openJioRequests`
      );
      ...
      if (snapshot.exists()) {
        ...
        const requestsToDelete = allRequests.filter(
        ...
        for (const request of requestsToDelete) {
          await remove(
            ref(
              FIREBASE_DB,
              `users/${currentUserUid}/openJioRequests/${request.id}`
            )
          );
        }
      ...
      if (snapshot.exists()) {
        ...
        for (const request of requestsToDelete) {
          await remove(
            ref(
              FIREBASE_DB,
              `users/${currentUser.uid}/openJioRequests/${request.id}`
            )
          );
        }
      ...
      <View key={request.id} style={styles.requestItem}>
        <View style={styles.requestContent}>
          ...
          <Text style={styles.requestText}>
            <Text style={styles.boldText}>Open Jio</Text> with{" "}
            {request.requesterUsername}
          </Text>
        </View>
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            onPress={() => handleOpenJioAccept(request.id)}
